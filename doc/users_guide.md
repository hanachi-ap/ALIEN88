...
- モッドタップ
  ...
  モッドタップ は、押したままの時にモディファイアのように機能し、タップされた時に通常のキーのように振舞います。別の言い方をすると、タップした時に SPACE を送信しますが、押したままの時に Control あるいは Shift キーとして機能するキーを持つことができます。ああ
...
## 2． 言語環境について
ですのでALIEN88は物理的な配置としては英語配列をベースにしていますが、理論配列は 英語、日本語
あるいは他の言語設定でも問題なく利用することができます。

<もう少し解説>

接続しているキーボードを英語配列としてつかうか日本語配列として使うかはOS側が制御しています。
